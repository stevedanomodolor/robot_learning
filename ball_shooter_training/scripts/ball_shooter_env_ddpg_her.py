...
from ball_shooter_utils_rl_ddpg_her import BallShooterRLUtilsDdpgHer

#register the environment to openai
reg = register(
    id='BallShooterEnvDdpgHer-v0',
    entry_point='ball_shooter_env_ddpg_her:BallShooterEnvDdpgHer',
    max_episode_steps=50, # this should be one in thoery
    )

class BallShooterEnvDdpgHer(gym.Env):
    def __init__(self):
        # obtain parameters
        ...
        self.n_actions = rospy.get_param("/ball_shooter/n_actions_ddpg")
        ...
        # ball shooter object
        self.ball_shooter_object = BallShooterRLUtilsDdpgHer()
    ...
    def step(self,action):
        # #print("actions: " + str(type(action[0])) + "len: " + str(len(action[0])))
        # #print("actions: " + str(action[0]))
        action_array = action[0]
        #print("actions----: " + str(action_array[0]))
        #print("actions----: " + str(action_array[1]))
        ...
        self.ball_shooter_object.launch_ball(vel_cmd = action_array[0])
    ...
    def reset(self):
        ...
        rospy.logdebug("Resetting SIM..")
        self.gazebo.resetSim()
        # Reset controller
        rospy.logdebug("Reset pan_joint_position_controller..")
        self.controllers_object.reset_ball_shooter_joint_controllers()
        #reset robot to intial position
        rospy.logdebug("Setting initial position")
        ...
        rospy.logdebug("check_all_systems_subscribers_publishers_services_ready...")
        ...
        rospy.logdebug("Pausing simulation...")
        self.gazebo.pauseSim()
        # get the current state
        state_ = self.ball_shooter_object.get_state()
